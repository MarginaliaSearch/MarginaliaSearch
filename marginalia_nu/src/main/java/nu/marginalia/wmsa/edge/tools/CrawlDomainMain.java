...
public class CrawlDomainMain {
    ...
    public static void main(String... args) {
        ...
        final Thread uploadThread = new Thread(CrawlDomainMain::uploadThread, "Uploader");
        uploadThread.start();
        ...
        try (var br = new BufferedReader(new InputStreamReader(System.in))) {
            for (;;) {
                var urlStr = br.readLine();
                if (urlStr == null) {
                    break;
                } else if (!urlStr.isBlank()) {
                    urls.add(new EdgeUrl(urlStr));
                }
            }
        }
        uploadFacade.putUrls(urls, -5);

        try (var c = conn.getConnection();
             var fetchDomains = c.prepareStatement("select ID FROM EC_DOMAIN WHERE URL_PART=?");
            ...
            fetchDomains.setFetchSize(1000);

            for (var url : urls) {
                fetchDomains.setString(1, url.getDomain().toString());
...
outer:
            while (AbortMonitor.getInstance().isAlive() && (running || !processQueue.isEmpty())) {
                ...
                if (!probe.ok()) {
                    ...
                    int countProp = Objects.requireNonNullElse(Integer.getInteger("wmsa.edge.crawl.maxCeiling"), 0);

                    int countMaxAll = 4000;
                    int countVisited = (int) (job.visitedCount * 1.1);
                    int countHash = 100 + job.hashes.size() * 2;

                    int maxCount = Math.max(countProp, Math.min(countMaxAll, Math.max(countVisited, countHash)));
...
outer:
        while (AbortMonitor.getInstance().isAlive() && (running || !processQueue.isEmpty() || !uploadQueue.isEmpty())) {
            ...
            var newContents = visits.stream().filter(visit -> {
