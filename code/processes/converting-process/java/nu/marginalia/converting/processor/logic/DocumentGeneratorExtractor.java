...
public class DocumentGeneratorExtractor {
    ...
    public DocumentGenerator detectGenerator(EdgeUrl url, Document doc, DocumentHeaders responseHeaders, DocumentTags tags) {
        ...
        var generatorTags = new ArrayList<Element>();
        for (var meta : tags.metaTags()) {
            if (DocumentTags.attrIs(meta, "name", "generator")) {
                generatorTags.add(meta);
            }
        }

        if (generatorTags.isEmpty()) {
            // Some sites have a comment in the head instead of a meta tag
            return fingerprintServerTech(doc, responseHeaders);
        }
        if (generatorTags.size() > 1) {
            return DocumentGenerator.multiple();
        }
        String generator = generatorTags.getFirst().attr("content");
    ...
    private static final Map<String, String> generatorByElementId = Map.of(
            "flarum-json-payload", "flarum",
            "tracpowered", "trac",
            "_xfClientLoadTime", "xenforo",
            "___gatsby", "gatsby"
    );
    private static final Map<String, String> generatorByClassName = Map.of(
            "ipsApp", "invision",
            "lemmy-site", "lemmy"
    );
    ...
    private DocumentGenerator fingerprintServerTech(Document doc, DocumentHeaders responseHeaders) {
        ...
        for (var node : doc.head().childNodes()) {
        ...
        String platformMarker = findPlatformMarker(doc);
        if (platformMarker != null) {
            return DocumentGenerator.of(platformMarker);
        }
    ...
    @Nullable
    private String findPlatformMarker(Document doc) {
        String[] result = new String[1];

        doc.filter((node, depth) -> {
            if (!(node instanceof Element el))
                return NodeFilter.FilterResult.CONTINUE;

            String generator = generatorByElementId.get(el.id());

            if (generator == null) {
                for (var entry : generatorByClassName.entrySet()) {
                    if (el.hasClass(entry.getKey())) {
