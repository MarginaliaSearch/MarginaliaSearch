...
import nu.marginalia.ranking.connectivity.DomainSetConnectivity;
...
/** Calculates the connectivity map, which classifies each indexed domain by how it connects
 * to the domains in the origin ranking set.  The result is persisted to the search sets
 * directory, where the index service picks it up.
 */
public class ConnectivitySetsCalculator {

    private static final Logger logger = LoggerFactory.getLogger(ConnectivitySetsCalculator.class);
    ...
    @Inject
    public ConnectivitySetsCalculator(AggregateLinkGraphClient graphClient,
        ...
        filePath = DomainRankingSetsService.setFileName(searchSetsBase, ConnectivityMapFile.FILE_NAME);
    }

    public ConnectivityView recalculate() {
        ...
        try {
            ...
            var originSet = new RankingSearchSet(sourceSetName,
                    DomainRankingSetsService.setFileName(searchSetsBase, sourceSetName));
            ...
            ConnectivityMapFile.write(filePath, newConnectivity);

            return new ConnectivityView(newConnectivity);
        }
        catch (SQLException | IOException ex) {
            logger.error("Failed to calculate connectivity sets", ex);

            return loadPrevious();
        }
    }

    private ConnectivityView loadPrevious() {
        try {
            return new ConnectivityView(ConnectivityMapFile.read(filePath));
        }
        catch (IOException ex) {
            return new ConnectivityView(new Int2ByteOpenHashMap());
        }
    }
