...
public class SkipListReaderTest {
    ...
    public void testRetainTenBlocks() throws IOException {
        long[] keys = LongStream.range(0, 320000).map(v -> 2*v).toArray();
        long[] vals = LongStream.range(0, 320000).map(v -> -v).toArray();
    ...
    public void testRetainBug() throws IOException {
        ...
        try (var writer = new SkipListWriter(docsFile, valuesFile)) {
            ...
            start = writer.writeList(createArray(keys, vals), keys.length);
        }

        try (var indexPool = new BufferPool(docsFile, SkipListConstants.BLOCK_SIZE, 8);
             var valueReader = new SkipListValueReader(valuesFile)) {
            var reader = new SkipListReader(indexPool, valueReader,  start);
    ...
    public void getKeys2() throws IOException {
        ...
        long pos = 0;
        try (var writer = new SkipListWriter(docsFile, valuesFile)) {
            pos = writer.writeList(createArray(keys, vals),  keys.length);
            writer.writeList(createArray(keys, vals),  keys.length);
        }

        try (var indexPool = new BufferPool(docsFile, SkipListConstants.BLOCK_SIZE, 8);
             var valueReader = new SkipListValueReader(valuesFile)) {
            var reader = new SkipListReader(indexPool, valueReader,  pos);
    ...
    public void testWtf() {
        ...
        try (LongArray array = LongArrayFactory.onHeapConfined(4096);
            ...
            long pos = writer.writeList(array.shifted(513), 255*2);
